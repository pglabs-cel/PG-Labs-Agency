...
const MAX_DIMENSION = 10000; // 10k pixels max dimension (Requirement 4)
const MAX_TOTAL_PIXELS = 89000000; // ~89 Megapixels decompression bomb protection ceiling

export class ImageProcessingService {
  /**
   * Validates pixel ceiling before full decode, then re-encodes through a clean
   * raster pipeline to strip all EXIF/GPS metadata and neutralize polyglots. (Requirement 4 & 5)
   */
  public static async processAndSanitizeImage(
    ...
    // Sharp strips all EXIF, GPS, comments, and polyglots unless withMetadata() is explicitly called.
