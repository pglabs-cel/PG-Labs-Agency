/**
 * Magic-Byte / Binary Signature Verification
...
export function verifyFileSignature(buffer: Buffer): FileSignatureResult {
  ...
  // Usually bytes 4-7 are 'ftyp' or 'moov'
  ...
  if (
    ...
    return {
      isValid: false,
      error: "SVG uploads are disabled for security reasons (XSS & script injection prevention). Please use WebP, PNG, or JPG.",
    };
  }

  return {
    isValid: false,
    error: "Invalid file signature. File header does not match any allowed image or video format.",
  };
}
